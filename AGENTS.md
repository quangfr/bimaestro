...
3. **Édition de `index.html` (+2100 lignes) :**
   ...
   - Les panneaux de données tabulaires d'Étapes 3 et 5 (`step-3-table-panel`, `step-5-table-panel`) disposent d'un sélecteur à deux vues (`ui`, `<>`) : `ui` affiche le tableau dynamique de calcul (`step-X-ui-view`) et `<>` affiche le prompt IA Markdown standardisé de suggestions comparatives de visuels (`step-X-ai-view`, `copyTableAi(step)`), personnalisable via `openPromptTemplateModal(step)`.
   - Les panneaux graphiques d'Étapes 4 et 6 (`step-4-chart-panel`, `step-6-chart-panel`) disposent d'un sélecteur à trois vues (`ui`, `js`, `<>`) : `ui` affiche le rendu canvas Chart.js actif (`step-X-chart-view`), `js` présente un bloc de code préformaté et copiable (`step-X-js-view`, `copyChartDataJs(step)`) contenant la configuration complète de l'objet Chart.js ({ type, data, options, plugins }) de la visualisation active, et `<>` affiche le prompt IA Markdown standardisé copiable (`step-X-ai-view`, `copyChartAi(step)`).
